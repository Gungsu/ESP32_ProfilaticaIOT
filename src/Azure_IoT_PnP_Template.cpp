...
rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_OPCAODOSAGEM));
  ...
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding Concetracao property name to telemetry payload.");
  rc = az_json_writer_append_int32(&jw, leituraProfsys.Concetracao);
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding Concetracao property value to telemetry payload.");
